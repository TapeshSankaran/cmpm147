    ...
    return [
        {
            name: "Normal World",
            functions: {
                drawTile: p3_drawNormalTile, 
                drawAfter: p3_normalDrawAfter
            }
        },
        {
            name: "Infernal World",
            functions: {
                drawTile: p3_drawLavaTile, 
                drawAfter: p3_lavaDrawAfter
            }
        },
        {
            name: "Cold World",
            functions: {
                drawTile: p3_drawSnowyTile, 
                drawAfter: p3_snowyDrawAfter
            }
        }
    ]
} 
...
function preload() {
  ...
  for (let i = 0; i < worlds.length; i++) {
    let name = worlds[i].name;
    let option = document.createElement("option");
    option.value = i;
    option.innerHTML = name;
    dropper.append(option);
  }
  dropper.change(function (e) {
    p3_worldChanged(worlds[e.target.value]); 
  });
...
function setup() {
  ...
  label.parent("otherContainer");

  let inputLabel = createInput("xyzzy");
  inputLabel.parent(label);
  inputLabel.input(() => {
    rebuildWorld(inputLabel.value());
  });

  createP("Arrow keys and WASD to scroll.").parent("otherContainer");
  createP("Clicking changes some tiles.").parent("otherContainer");
...
function draw() {
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      drawTile(tileRenderingOrder([x + world_offset.x, y - world_offset.y], m), [
        camera_offset.x,
        camera_offset.y
      ]); // odd row
    }
  ...
  if (window.p3_drawAfter) {
    window.p3_drawAfter(camera_offset.x*2, camera_offset.y*2);
  }
}
...
function drawTile([world_x, world_y], [camera_x, camera_y], m) {
  let [screen_x, screen_y] = worldToScreen(
    [world_x, world_y],
    [camera_x, camera_y]
  );
  ...
  if (window.p3_drawTile) {
    window.p3_drawTile(world_x, world_y, -screen_x, screen_y, 8*window.p3_tileRandom(world_x, world_y, 4));
  }
  pop();
}
